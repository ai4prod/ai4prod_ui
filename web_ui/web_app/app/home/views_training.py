...
def start_training():
    ...
    conf = Configuration.query.filter(
        Configuration.id == dataset_query.conf_id).first()

    print(conf.task)

    configurationHandler.update_conf_before_training(
        task=conf.task,
...
def move_specific_keys_to_first(dictionary,
    ...
    moved_items = [(key, dictionary[key])
                   for key in keys_to_move if key in dictionary]
    remaining_items = []
    if include_others:
        remaining_items = [
            (key, value) for key, value in dictionary.items() if key not in keys_to_move]
    ...
    for run_uuid in run_uuids:
        ...
        if (tmp_dict["dataset_id"] == str(dataset_id)):
            ...
            keys_to_move = ['experiment_number', 'dataset_version',
                            "dataset_path", "val_accuracy", "val_cross_entropy", "dataset_id"]
